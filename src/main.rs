use std::sync::Arc;
use arrow::{array::{ArrayRef, Float64Array, Int32Array, TimestampSecondArray}, record_batch::RecordBatch};
use chrono::{DateTime, Utc};

use arrow::{self, array::StringArray, datatypes::{Field, DataType, TimeUnit, Schema}};
use rusqlite::{self, Connection};

fn get_data_type(sql_name: Option<&str>) -> DataType {
    sql_name.map(|name| match name {
            "TEXT" => DataType::Utf8,
...
fn get_schema(connection: &Connection, table_name: &str) -> rusqlite::Result<Schema> {
    ...
    let fields: Vec<Field> = columns.into_iter().map(|column| {
        Field::new(column.name(), get_data_type(column.decl_type()), false)
    }).collect();
...
fn main() -> rusqlite::Result<()> {
    ...
    let names_and_types: Vec<(&String, &DataType)> = schema.fields().iter().map(|field| (field.name(), field.data_type())).collect();

    let arrays: Vec<Arc<(dyn arrow::array::Array + 'static)>> = names_and_types.into_iter().map(|(field_name, field_type)|{
